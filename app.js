...
function getDistrictFromFeature(feature) {
  ...
  // Always match against the federal list — it's the one whose keys are the
  // real, stable province names found in the GeoJSON. Chamber-specific district
  // keys (e.g. "Brussels-FR") never appear in the map data itself; translating
  // this canonical name into the active chamber's own district key is the job
  // of resolveDistrictKeyForMapKey(), not this function.
  const canonicalDistricts = CHAMBERS.federal.districts;
  if (name && canonicalDistricts[name]) return name;

  const slug = slugDistrict(name || "");
  for (const [key, info] of Object.entries(canonicalDistricts)) {
  ...
  geoJsonLayer.eachLayer(layer => {
    ...
    if (podium.length && podium[0].value > 0) {
      ...
      if (podium.length >= 2) {
        ...
        podiumLines += `<br><em>écart : +${margin.toFixed(1)} pt</em>`;
      }
    }
